...
FerrariSans is licensed. Open-source substitute: **Inter** at weight 500 with letter-spacing -1%, or **S철hne** for closer humanist proportions.

## Layout

### Spacing System
- **Base unit:** 4px.
- **Tokens:** `{spacing.xxxs}` 4px 쨌 `{spacing.xxs}` 8px 쨌 `{spacing.xs}` 16px 쨌 `{spacing.sm}` 24px 쨌 `{spacing.md}` 32px 쨌 `{spacing.lg}` 48px 쨌 `{spacing.xl}` 64px 쨌 `{spacing.xxl}` 96px 쨌 `{spacing.super}` 128px.
...
**`button-primary`**  - The signature Rosso Corsa CTA. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}` (14px / 700 / 1.4px tracking, uppercase), padding 14px x 32px, height 48px, **rounded `{rounded.none}` (0px  - sharp corners)**.
...
**`text-input-on-dark`**  - Background `{colors.canvas}`, text `{colors.ink}`, rounded `{rounded.sm}` (4px), padding 14px x 16px, height 48px, 1px `{colors.hairline}` border.

**`text-input-on-light`**  - White-canvas variant.

**`badge-pill`**  - Small uppercase pill. Background `{colors.canvas-elevated}`, text `{colors.ink}`, type `{typography.caption-uppercase}` (11px / 600 / 1.1px tracking, uppercase), rounded `{rounded.full}` (9999px), padding 4px x 12px. The only place pill geometry is used.
...
**`footer-dark`**  - Closing dark footer. Background `{colors.canvas}`, text `{colors.body}`. 5-column link list. 64x48px padding.
...
|---|---|---|
| Mobile | < 640px | Hero photograph crops vertically; hero h1 80 - 2px; feature card grid 1-up; nav hamburger; preowned listing 1-up. |
| Tablet | 640 - 024px | Hero h1 56px; feature card grid 2-up; preowned listing 2-up. |
| Desktop | 1024 - 280px | Full hero h1 80px; feature card grid 3-up; preowned listing 4-up. |
| Wide | > 1280px | Editorial body content caps at 1280px; hero photography continues full-bleed. |

### Touch Targets
- Primary CTA at 48px height  - at WCAG AAA (44 x 44).
